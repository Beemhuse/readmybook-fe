...
var startSpeech = function() {
    ...
    if (!speaking || paused) {
        if (paused) {
            ...
            responsiveVoice.speak(info, getSelectedVoiceName(), {rate: speechSpeed});
        }
        speaking = true;
        paused = false;
    }
...
var pauseSpeech = function() {
    if (speaking) {
        responsiveVoice.pause();
        speaking = false;
    }
}
var resumeSpeech = function() {
    if (speaking && paused) {
        responsiveVoice.resume();
        paused = false;
    }
...
var getSelectedVoiceName = function() {
    switch(nativeVoice){
        case "1":
        ...
        default:
            return "UK English Female"; // Change the default voice if needed
    }
}
...
document.getElementById("play-button").addEventListener("click", startSpeech);
